...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagbeep)
    {
        bitFieldBitLen += beep_LEN;
    }
    ...
    if(0x01 == aFlag->flagmode)
    {
        bitFieldBitLen += mode_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagbeep)
    {
        info->event[info->num] = EVENT_beep;
        info->num++;
        dataPoints->valuebeep = gizVarlenDecompressionValue(bitFieldOffset,beep_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += beep_LEN;
    }
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagmode)
    {
        info->event[info->num] = EVENT_mode;
        info->num++;
        dataPoints->valuemode = gizVarlenDecompressionValue(bitFieldOffset,mode_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagvoltageValue)
    {
        info->event[info->num] = EVENT_voltageValue;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuevoltageValue,(uint8_t *)&issuedData[byteFieldOffset],voltageValue_LEN);
        byteFieldOffset += voltageValue_LEN;
        dataPoints->valuevoltageValue = gizX2YFloat(voltageValue_RATIO,  voltageValue_ADDITION, (float)(elongateP0FormTmp.devStatus.valuevoltageValue));
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagpv_currentValue)
    {
        info->event[info->num] = EVENT_pv_currentValue;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuepv_currentValue,(uint8_t *)&issuedData[byteFieldOffset],pv_currentValue_LEN);
        byteFieldOffset += pv_currentValue_LEN;
        dataPoints->valuepv_currentValue = gizX2Y(pv_currentValue_RATIO,  pv_currentValue_ADDITION, elongateP0FormTmp.devStatus.valuepv_currentValue); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuebeep != cur->valuebeep)
    {
     //   GIZWITS_LOG("valuebeep Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagbeep = 1;
        ret = 1;
    }
    ...
    if(last->valuevoltageValue != cur->valuevoltageValue)
    {
     //   GIZWITS_LOG("valuevoltageValue Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagvoltageValue = 1;
        ret = 1;
    }
    if(last->valuepv_currentValue != cur->valuepv_currentValue)
    {
     //   GIZWITS_LOG("valuepv_currentValue Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagpv_currentValue = 1;
        ret = 1;
    }
    ...
    if(last->valuepv_voltage != cur->valuepv_voltage)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
         //   GIZWITS_LOG("valuepv_voltage Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagpv_voltage = 1;
            ret = 1;
        }
    }
    if(last->valuepv_efficiency != cur->valuepv_efficiency)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
         //   GIZWITS_LOG("valuepv_efficiency Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagpv_efficiency = 1;
            ret = 1;
        }
    }
    if(last->valuepv_power != cur->valuepv_power)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
          //  GIZWITS_LOG("valuepv_power Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagpv_power = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagbeep)
    {
        gizVarlenCompressValue(bitFieldOffset,beep_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuebeep);
        bitFieldOffset += beep_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagvoltageValue)
    {
        devStatusTmp.valuevoltageValue = gizY2XFloat(voltageValue_RATIO,  voltageValue_ADDITION, dataPoints->valuevoltageValue); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuevoltageValue,voltageValue_LEN);
        byteFieldOffset += voltageValue_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagpv_currentValue)
    {
        devStatusTmp.valuepv_currentValue = gizY2X(pv_currentValue_RATIO,  pv_currentValue_ADDITION, dataPoints->valuepv_currentValue); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuepv_currentValue,pv_currentValue_LEN);
        byteFieldOffset += pv_currentValue_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagpv_voltage)
    {
        devStatusTmp.valuepv_voltage = exchangeBytes(gizY2XFloat(pv_voltage_RATIO,  pv_voltage_ADDITION, dataPoints->valuepv_voltage));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuepv_voltage,pv_voltage_LEN);
        byteFieldOffset += pv_voltage_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagpv_efficiency)
    {
        devStatusTmp.valuepv_efficiency = exchangeBytes(gizY2XFloat(pv_efficiency_RATIO,  pv_efficiency_ADDITION, dataPoints->valuepv_efficiency));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuepv_efficiency,pv_efficiency_LEN);
        byteFieldOffset += pv_efficiency_LEN;
    }

    if(gizwitsProtocol.waitReportDatapointFlag.flagpv_power)
    {
        devStatusTmp.valuepv_power = exchangeWord(gizY2XFloat(pv_power_RATIO,  pv_power_ADDITION, dataPoints->valuepv_power)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuepv_power,pv_power_LEN);
        byteFieldOffset += pv_power_LEN;
    }
