...
uint16_t	key3_cnt;					//CNT计数标志（消抖+长按计时）
uint8_t		key3_short_flag;	//短按标志
uint8_t		key3_lock_flag;		//长按-锁标志
uint8_t		key3_long_flag;		//长按-标志（与KEY1一致）
...
/*-------------------------------------------------------------------------------------------------*/	
		...
		/*按键松手/按键未按下*/
	if(KEY3) 												   //如果没有按键按下（或松手）
{
	key3_lock_flag = 0;					     //清零自锁标志
	key3_cnt = 0; 								   //清零计数标志（消抖+长按计时）
	if(key3_short_flag)
	{
		key3_short_flag = 0;			     //清零短按标志位
		KeyNum = KEY3_Short; 					 //赋键值编码 短按键值
	}
	if(key3_long_flag)
	{
		key3_long_flag = 0;						  //清零长按标志位
		KeyNum = KEY3_Long; 						//赋键值编码 长按键值
	}
}
...
{
	key3_cnt++; 											//累计按键消抖延时次数
	if(key3_cnt > KEY_DELAY_TIME)		// KEY_DELAY_TIME--消抖延时（与KEY1共用同一个宏）
	{
		key3_cnt = 0; 									//清零计数标志
		key3_short_flag = 1;						//短按标志位置1
		key3_long_flag = 0;							//长按标志位清0
...
else
{
	...
	if(key3_cnt > KEY_LONG_TIME	)			//KEY_LONG_TIME--长按阈值（与KEY1共用同一个宏）
	{
		key3_short_flag = 0;						//短按标志位清0（切换为长按）
		key3_long_flag = 1;							//长按标志位置1
	}
}
...
/*-------------------------------------------------------------------------------------------------*/		
	...
	if(KEY4) 												   //如果没有按键按下（或松手）
{
	key4_lock_flag = 0;					     //清零自锁标志
	key4_cnt = 0; 								   //清零计数标志（消抖+长按计时）
	if(key4_short_flag)
	{
		key4_short_flag = 0;			     //清零短按标志位
		KeyNum = KEY4_Short; 					 //赋键值编码 短按键值
	}
	if(key4_long_flag)
	{
		key4_long_flag = 0;						  //清零长按标志位
		KeyNum = KEY4_Long; 						//赋键值编码 长按键值
	}
}
...
{
	key4_cnt++; 											//累计按键消抖延时次数
	if(key4_cnt > KEY_DELAY_TIME)		// KEY_DELAY_TIME--消抖延时（与KEY1共用同一个宏）
	{
		key4_cnt = 0; 									//清零计数标志
		key4_short_flag = 1;						//短按标志位置1
		key4_long_flag = 0;							//长按标志位清0
...
else
{
	...
	if(key4_cnt > KEY_LONG_TIME	)			//KEY_LONG_TIME--长按阈值（与KEY1共用同一个宏）
	{
		key4_short_flag = 0;						//短按标志位清0（切换为长按）
		key4_long_flag = 1;							//长按标志位置1
	}
}
}
